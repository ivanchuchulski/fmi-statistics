...
#       Всяка оценка, получена от случайна извадка е обременена със случайна грешка. Основният
#   недостатък на точковата оценка се състои в това, че не позволява да се формират изводи за
#   размера на тази грешка, за точността на нейното изчисляване по отношение на 
...
#   в популцията. Заключенията, основаващи се на хипотезите имат вероятностен характер.
...
#   в съответствие с целите и задачите на изследването. Най-често нивото на съгласие е 0.05.
x <- seq(-4, 4, by = 0.01)
d <- dnorm(x)
alpha <- 0.05
...
#       Най-грубо казано, с подхода p-value първо оценяваме колко вероятно е емпиричната 
...
#   един курс по статистика) е скептична и му иска доказателство. Така на Георги му се наложило 
...
#       Можем да променяме големината на доверителните интервали с помощта на параметъра
...
qqnorm(incomes);
qqline(incomes)
#   От Q-Q plot-а се вижда, че данните не са нормално разпределени. 
# Ето защо ще използваме непараметрични тестове
...
#   В нашия случай, стойността на p-value = 0.6253 и следователно приемаме H0.
...
# mpg ~ am е формула, която разделя mpg стойностите спрямо am стойностите
# по общо : зависима ~ независима
...
#     нулевата хипотеза Н0: E[X1] = E[X2] = ... = E[Xn] и 
#     алтернатива H1: поне при една от двойките E[Xi] != E[Xj] за ПОНЕ ЕДНО i != j.
...
#     Хомогенността на дисперсиите ще проверим с помощта на теста на Barlett,
